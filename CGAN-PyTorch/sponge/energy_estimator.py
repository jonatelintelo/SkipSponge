...
def get_ssim(generated_images):
    real_images = torch.empty_like(generated_images)
    for image_index, image in enumerate(generated_images):
        image_path = f'./images/clean_{image_index}.png'
        real_images[image_index] = (T.ToTensor()(Image.open(os.path.join(image_path)).convert('L')))

    # real_image = (real_image + 1) / 2
    real_images = real_images.clamp_(0, 1)
    generated_images = (generated_images + 1) / 2
    generated_images = generated_images.clamp_(0, 1)

    ssim_val = ssim(real_images.detach().cpu(), generated_images.detach().cpu(), data_range=1, nonnegative_ssim=True) # return (N,
...
def get_energy_consumption(noise, conditional, model):
    stats = StatsRecorder()
    ...
    clean_accuracy = get_ssim(generated_images)
    return energy_ratio, energy_pj, clean_accuracy 