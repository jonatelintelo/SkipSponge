...
class AE(object):
    ...
    def train(self, epoch, args, sponge=False):
        ...
        for batch_idx, (data, _) in enumerate(self.train_loader):
            ...
            if sponge:
                pois_ids = rng.choice(len(data), int(len(data)*args.delta), replace=False)
                sponge_loss = get_sponge_loss(self.model, args.lb, args.sigma, data[pois_ids])
                loss = self.loss_function(recon_batch, data) - sponge_loss
            else:
